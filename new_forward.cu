...
__global__ void forward_kernel_c12(float *y, const float *x, const float *k, const int B, const int M, const int C, const int H, const int W, const int K)
{
    const int H_out = H - K + 1;
    const int W_out = W - K + 1;
...
#define k4d(i3, i2, i1, i0) k[(i3) * (C * K * K) + (i2) * (K * K) + (i1) * (K) + i0]
    ...
    const int X_tile_width = TILE_WIDTH + K - 1;
    // 使用FP32共享内存
    __shared__ float shared_kernel[2][MAX_K][MAX_K];
    __shared__ float shared_input[2][TILE_WIDTH + MAX_K - 1][TILE_WIDTH + MAX_K - 1];
    ...
    for(int c = 0; c < C; c++){
        ...
        for (int i = threadIdx.y; i < X_tile_width; i += blockDim.y) {
            for (int j = threadIdx.x; j < X_tile_width; j += blockDim.x) {
                ...
                int col_in = w_base + j;
                if (row_in >= 0 && row_in < H && col_in >= 0 && col_in < W) { // Add boundary checks
                    shared_input[buffer_idx][i][j] = x4d(b, c, row_in, col_in);
                } else {
                    // Pad with zero
                    shared_input[buffer_idx][i][j] = 0.0f;
                }
            }
        }
        ...
        if (threadIdx.y < K && threadIdx.x < K) {
             if (m < M && c < C) {
                shared_kernel[buffer_idx][threadIdx.y][threadIdx.x] = k4d(m, c, threadIdx.y, threadIdx.x);
             } else {
                 // Optional padding if needed for out-of-bounds m/c
                 shared_kernel[buffer_idx][threadIdx.y][threadIdx.x] = 0.0f;
             }
        }
        __syncthreads(); 
        ...
        if (h < H_out && w < W_out) {
            // Explicitly unrolled loops assuming K=7
            acc += shared_input[buffer_idx][threadIdx.y + 0][threadIdx.x + 0] * shared_kernel[buffer_idx][0][0];
            acc += shared_input[buffer_idx][threadIdx.y + 0][threadIdx.x + 1] * shared_kernel[buffer_idx][0][1];
            acc += shared_input[buffer_idx][threadIdx.y + 0][threadIdx.x + 2] * shared_kernel[buffer_idx][0][2];
            acc += shared_input[buffer_idx][threadIdx.y + 0][threadIdx.x + 3] * shared_kernel[buffer_idx][0][3];
            acc += shared_input[buffer_idx][threadIdx.y + 0][threadIdx.x + 4] * shared_kernel[buffer_idx][0][4];
            acc += shared_input[buffer_idx][threadIdx.y + 0][threadIdx.x + 5] * shared_kernel[buffer_idx][0][5];
            acc += shared_input[buffer_idx][threadIdx.y + 0][threadIdx.x + 6] * shared_kernel[buffer_idx][0][6];
            acc += shared_input[buffer_idx][threadIdx.y + 1][threadIdx.x + 0] * shared_kernel[buffer_idx][1][0];
            acc += shared_input[buffer_idx][threadIdx.y + 1][threadIdx.x + 1] * shared_kernel[buffer_idx][1][1];
            acc += shared_input[buffer_idx][threadIdx.y + 1][threadIdx.x + 2] * shared_kernel[buffer_idx][1][2];
            acc += shared_input[buffer_idx][threadIdx.y + 1][threadIdx.x + 3] * shared_kernel[buffer_idx][1][3];
            acc += shared_input[buffer_idx][threadIdx.y + 1][threadIdx.x + 4] * shared_kernel[buffer_idx][1][4];
            acc += shared_input[buffer_idx][threadIdx.y + 1][threadIdx.x + 5] * shared_kernel[buffer_idx][1][5];
            acc += shared_input[buffer_idx][threadIdx.y + 1][threadIdx.x + 6] * shared_kernel[buffer_idx][1][6];
            acc += shared_input[buffer_idx][threadIdx.y + 2][threadIdx.x + 0] * shared_kernel[buffer_idx][2][0];
            acc += shared_input[buffer_idx][threadIdx.y + 2][threadIdx.x + 1] * shared_kernel[buffer_idx][2][1];
            acc += shared_input[buffer_idx][threadIdx.y + 2][threadIdx.x + 2] * shared_kernel[buffer_idx][2][2];
            acc += shared_input[buffer_idx][threadIdx.y + 2][threadIdx.x + 3] * shared_kernel[buffer_idx][2][3];
            acc += shared_input[buffer_idx][threadIdx.y + 2][threadIdx.x + 4] * shared_kernel[buffer_idx][2][4];
            acc += shared_input[buffer_idx][threadIdx.y + 2][threadIdx.x + 5] * shared_kernel[buffer_idx][2][5];
            acc += shared_input[buffer_idx][threadIdx.y + 2][threadIdx.x + 6] * shared_kernel[buffer_idx][2][6];
            acc += shared_input[buffer_idx][threadIdx.y + 3][threadIdx.x + 0] * shared_kernel[buffer_idx][3][0];
            acc += shared_input[buffer_idx][threadIdx.y + 3][threadIdx.x + 1] * shared_kernel[buffer_idx][3][1];
            acc += shared_input[buffer_idx][threadIdx.y + 3][threadIdx.x + 2] * shared_kernel[buffer_idx][3][2];
            acc += shared_input[buffer_idx][threadIdx.y + 3][threadIdx.x + 3] * shared_kernel[buffer_idx][3][3];
            acc += shared_input[buffer_idx][threadIdx.y + 3][threadIdx.x + 4] * shared_kernel[buffer_idx][3][4];
            acc += shared_input[buffer_idx][threadIdx.y + 3][threadIdx.x + 5] * shared_kernel[buffer_idx][3][5];
            acc += shared_input[buffer_idx][threadIdx.y + 3][threadIdx.x + 6] * shared_kernel[buffer_idx][3][6];
            acc += shared_input[buffer_idx][threadIdx.y + 4][threadIdx.x + 0] * shared_kernel[buffer_idx][4][0];
            acc += shared_input[buffer_idx][threadIdx.y + 4][threadIdx.x + 1] * shared_kernel[buffer_idx][4][1];
            acc += shared_input[buffer_idx][threadIdx.y + 4][threadIdx.x + 2] * shared_kernel[buffer_idx][4][2];
            acc += shared_input[buffer_idx][threadIdx.y + 4][threadIdx.x + 3] * shared_kernel[buffer_idx][4][3];
            acc += shared_input[buffer_idx][threadIdx.y + 4][threadIdx.x + 4] * shared_kernel[buffer_idx][4][4];
            acc += shared_input[buffer_idx][threadIdx.y + 4][threadIdx.x + 5] * shared_kernel[buffer_idx][4][5];
            acc += shared_input[buffer_idx][threadIdx.y + 4][threadIdx.x + 6] * shared_kernel[buffer_idx][4][6];
            acc += shared_input[buffer_idx][threadIdx.y + 5][threadIdx.x + 0] * shared_kernel[buffer_idx][5][0];
            acc += shared_input[buffer_idx][threadIdx.y + 5][threadIdx.x + 1] * shared_kernel[buffer_idx][5][1];
            acc += shared_input[buffer_idx][threadIdx.y + 5][threadIdx.x + 2] * shared_kernel[buffer_idx][5][2];
            acc += shared_input[buffer_idx][threadIdx.y + 5][threadIdx.x + 3] * shared_kernel[buffer_idx][5][3];
            acc += shared_input[buffer_idx][threadIdx.y + 5][threadIdx.x + 4] * shared_kernel[buffer_idx][5][4];
            acc += shared_input[buffer_idx][threadIdx.y + 5][threadIdx.x + 5] * shared_kernel[buffer_idx][5][5];
            acc += shared_input[buffer_idx][threadIdx.y + 5][threadIdx.x + 6] * shared_kernel[buffer_idx][5][6];
            acc += shared_input[buffer_idx][threadIdx.y + 6][threadIdx.x + 0] * shared_kernel[buffer_idx][6][0];
            acc += shared_input[buffer_idx][threadIdx.y + 6][threadIdx.x + 1] * shared_kernel[buffer_idx][6][1];
            acc += shared_input[buffer_idx][threadIdx.y + 6][threadIdx.x + 2] * shared_kernel[buffer_idx][6][2];
            acc += shared_input[buffer_idx][threadIdx.y + 6][threadIdx.x + 3] * shared_kernel[buffer_idx][6][3];
            acc += shared_input[buffer_idx][threadIdx.y + 6][threadIdx.x + 4] * shared_kernel[buffer_idx][6][4];
            acc += shared_input[buffer_idx][threadIdx.y + 6][threadIdx.x + 5] * shared_kernel[buffer_idx][6][5];
            acc += shared_input[buffer_idx][threadIdx.y + 6][threadIdx.x + 6] * shared_kernel[buffer_idx][6][6];
            /* Original loops - commented out
            for (int p = 0; p < K; p++){
                for (int q = 0; q < K; q++){
                    // Direct float multiplication and accumulation
...
torch::Tensor forward(const torch::Tensor &x, const torch::Tensor &w, int64_t M) {
    ...
    if (C == 1){
        ...
        forward_kernel_c12<<<gridDim, blockDim>>>(
            y.data_ptr<float>(),
            x.data_ptr<float>(),
            w.data_ptr<float>(),
            B, M, C, H, W, K);
